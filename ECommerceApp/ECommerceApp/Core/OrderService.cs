...
namespace ECommerceApp.Core
{
    ...
    public class OrderService
    {
        ...
        public Order PlaceOrder(Cart cart, decimal discountPercent = 0)
        {
            ...
            order.ShippingFee = cart.CalculateShippingFee();
            order.FinalAmount += order.ShippingFee;
        ...
        /// <summary>
        /// Kupon kodu ile sipariş verme.
        /// Kupon geçerliliği kontrol edilir, geçerliyse indirim uygulanır.
        /// </summary>
        public Order PlaceOrderWithCoupon(Cart cart, Coupon coupon)
        {
            if (coupon == null)
                return PlaceOrder(cart, 0);
            ...
            if (!coupon.IsValid(total))
            {
                ...
                failOrder.OrderId = _nextOrderId++;
                failOrder.Status = OrderStatus.Failed;
                failOrder.FailReason = "Kupon geçersiz veya süresi dolmuş.";
                _orders.Add(failOrder);
                return failOrder;
            }

            var order = PlaceOrder(cart, coupon.DiscountPercent);
