...
> ⚠️ **Beklenen sonuç:** 21 test PASS, 6 test FAIL olacaktır. Fail olan testler bilerek bırakılmış bug'ları tespit etmektedir.
...
## 🐛 Bilerek Bırakılan Bug'lar (6 Adet)
...
|-----------|------|------|------|----------|
| **Unit Test (White Box)** | 7 | 5 | 2 | Kodun iç yapısı bilinerek yazılmış |
| **Black Box Test** | 9 | 7 | 2 | Sadece giriş/çıkış davranışına göre |
| **Gray Box Test** | 7 | 4 | 3 | Kısmi iç bilgi ile (sabitler, mesajlar) |
| **Integration Test** | 4 | 4 | 0 | Bileşenlerin birlikte çalışma testi |
| **TOPLAM** | **27** | **21** | **6** | — |
...
|--------|---------|-------|
| **Equivalence Partitioning (EP)** | 17 test | Geçerli: qty=3, Geçersiz: qty=0, qty=-1, null |
| **Boundary Value Analysis (BVA)** | 10 test | qty=Stock, qty=Stock+1, total=49/50/51 TL |
...
| **Regression Testing** | Tüm 27 test otomatize; bug fix sonrası tek komutla regression kontrolü |
